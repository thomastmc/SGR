# Simple Geometry of Reality (SGR)

**A Geometric Unification Framework for Time, Mass, and Dark Energy.**

> This repository contains the foundational manuscripts and derived applications of the SGR framework, which posits a 4-dimensional Riemannian manifold ($M$) with a kinematic foliation as the origin of physical observables.

---

## Repository Structure

### 1. [`/physics`](./physics)
* **Status:** Pure Theory (Independent Derivation)
* **Content:** Rigorous mathematical derivations of the Twist Invariant $\omega(t)$, the Wick Rotation mechanism, and the Soliton solutions for fermion identity.

### 2. [`/corollary`](./corollary)
* **Status:** Applied Physics & Structural Analysis
* **Content:** Applications, structural instantiations, and datasets. This includes an analysis of the 2025 Giza SAR tomography data.

---

**Author:** Thomas Michael Cunningham
*Independent Researcher*
